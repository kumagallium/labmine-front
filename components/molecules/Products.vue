...
export default {
  ...
  props: ['items','descriptions','defaults','notebook','is_condition','dataedit'],
  components: {
  },
  watch: {
  },
  ...
  computed: {
    setItems:{
      get() {
        ...
        if(this.descriptions != undefined){
          var defaults = this.defaults.filter(it => it.fields.is_condition == this.is_condition).map(it => it.fields.item_name)
          var description_tmp = this.descriptions.filter(it => it.fields.is_condition == this.is_condition).map(it => it.fields.item_name)
      ...
      set(val) {
        ...
        var defaults_tmp = this.defaults.filter(it => it.fields.is_condition == this.is_condition).map(it => it.fields.item_name)
        var description_tmp = this.descriptions.filter(it => it.fields.is_condition == this.is_condition).map(it => it.fields.item_name)
        ...
        if(description_tmp.length > 0){
          var hasvalues = this.descriptions.filter(it => it.fields.is_condition == this.is_condition).filter(des => des.fields.values.length > 0).map(des => des.fields.item)
    ...
    setDescriptions:{
      get() {
        var descriptions = []
        if(this.descriptions != undefined){
          descriptions = this.descriptions.filter(it => it.fields.is_condition == this.is_condition).map(it => it.fields.values)
        }
        return descriptions
      }
    },
